...
**Auto-setup:** First launch runs `.claude/cloud-install.sh` — installs .NET 9.0 SDK, the pinned Godot headless (version in `cloud-install.sh`), generates `.runsettings`.
...
**Adapted workflows:**
- Tests: **Must prefix `dotnet test` with `xvfb-run --auto-servernum`** — PreToolUse hook (`cloud_test_enforcer.py`) enforces automatically. GdUnit4 uses .NET Named Pipes; Godot needs an authenticated X display or the pipe server never starts.
- Code discovery: lead with semantic-search for "where is X" / prior-art; fall to Grep anchors for symbol precision. `.search-index/` is gitignored → run `/reindex_search` first on a fresh sandbox.
