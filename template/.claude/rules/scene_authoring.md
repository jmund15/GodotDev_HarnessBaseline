...
8. **A world-space visual is sized by the camera that sees it — and no test can see that.** `SpriteBase3D` and its `Label3D`/`Sprite3D` subclasses render `size × pixel_size` metres, depth-tested, with per-class defaults (`Label3D.pixel_size` 0.005, `SpriteBase3D` 0.01) chosen for neither this project's scale nor its orthographic camera. An unauthored world-space label is a fraction of a metre, occluded by the floor it spawns on, and **passes every suite that asserts its content, colour and lifecycle** — the failure is invisible to tests and to a reviewer reading two plausible numbers. Size it against a measured sibling already known readable in-game, and pin the DERIVED world height against a legibility floor rather than the authored number. *Litmus:* world height ÷ the run camera's `OrthoSize` — under ~2% is a handful of screen pixels. Live pin: `DamageFloaterTests.FloaterScene_IsLegibleUnderTheRunCamera`.
...
- Variable-count runtime composition (ability effect providers, per-wave enemies)
...
<!-- retire-when: review-by: 2027-02-24 -->
