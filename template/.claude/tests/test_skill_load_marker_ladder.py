...
def test_orchestration_load_injects_ladder_and_workflow_then_stays_quiet():
    ...
    # rows are asserted by family name and tier token; an effort or a model id would churn with the data
    assert "fable: orchestrator," in ctx and "opus: architect/executor," in ctx and "sonnet: fanout," in ctx, ctx
    assert "haiku: scout," in ctx, ctx
    assert not re.search(r"claude-[a-z]+-\d|gpt-\d", ctx), ctx
...
def test_workflow_without_the_skill_still_gets_the_fallback():
    ...
    assert "[role ladder" in ctx and "opus: architect/executor," in ctx and "(effort:" not in ctx, g
