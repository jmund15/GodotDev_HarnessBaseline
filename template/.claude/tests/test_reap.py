...
def run(argv, tbl, env=None, ledger=None, environ=None):
    killed = []
...
def main():
    ...
    rc, killed, out = run(["960", "--reason", "owner", "--why", "owner said"], table([envclean]))
    ...
    rc, killed, out = run(["103", "--reason", "owner", "--why", "user: kill the duplicate judge"], table())
    ...
    rc, killed, out = run(["53844", "--reason", "owner", "--why", "user: you close it, I am away from my desk"],
                          table([editor]))
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        rc, killed, out = run(["103", "--reason", "owner", "--why", "owner said"], table(), ledger=os.path.join(blocked, "reap_ledger.jsonl"))
    ...
    b = reap.git_bash() or ""
    ck("the environ read uses Git Bash, not WSL's System32\\bash.exe",
       os.path.basename(b).lower() == "bash.exe" and "system32" not in b.lower(), f"resolved {b!r}")
