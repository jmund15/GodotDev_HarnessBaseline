...
def main():
    ...
    if linked_jobs:
        ...
        # No symlink privilege (Windows without developer mode): the validator sees the jobs
        # path report itself as a symlink, which is the fact a real link would present.
        from pathlib import Path as _Path
        real_is_symlink = _Path.is_symlink
        with patch.object(_Path, "is_symlink",
                          lambda p: p.name == "jobs.json" or real_is_symlink(p)):
            linked_allowed = sidecar.fanout_allowed(
                {"cwd": fanout_root, "tool_input": {"command": fanout_cmd}})
