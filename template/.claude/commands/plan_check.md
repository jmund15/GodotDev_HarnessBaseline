...
Run independent lenses with explicit model, effort and agent profile through the existing engines. `orchestration` owns role/transport/currency selection. One suitable arm per needed lens; extra providers are not automatic work. Same-task comparisons require an explicit comparison request and budget.

| Shape | Required lenses |
|---|---|
| Code | `plc-memory-alignment`, `plc-pattern-fit`, `plc-architecture-quality`, `plc-test-readiness`, `plc-evidence-grounding` |
| Meta | `plc-memory-alignment`, `plc-instruction-quality`, `plc-doctrine-consistency`, `plc-evidence-grounding` |

Only pure retirement/rename code plans with no new types/files/exports/nodes/Jmodot code may omit pattern-fit and architecture-quality. State omissions and reasons. Add a lens only for a distinct uncovered risk. Judgment needs a qualified review role; size alone does not justify a higher effort.

Fetch mandates with `python3 .claude/tools/lens.py get --shared <keys>`. Do not load the whole mandate catalog. Put shared context and each mandate in files.
...
Follow the action protocol in [`agents/orchestrator_action_protocol.md`](agents/orchestrator_action_protocol.md): critical first, then `FIX|ASK|PLAN`, then `bug|rule|improvement`. Report the shape, lenses, scope gaps, verified findings and artifact path briefly. Account for every finding before acceptance.
...
Revise critical architecture findings and reconverge. At most two dispatched rounds; round two covers changed/failed lenses only. Verify mechanical folds inline; dispatch again only for new design or unresolved evidence. After two divergent rounds, surface the real fork rather than keep spending.
