...
4. **Load-bearing numbers with no provenance** — thresholds, ratios, token counts, timings, sizes. A number that decides a branch and appears from nowhere is ASSUMED, however plausible. **Repo cardinalities are the highest-yield sub-case** — files touched, implementers, call sites, inheritors, scene nodes — because a count *feels* like evidence and so bypasses the suspicion a bare assertion attracts. Their remedy is specific: not "verify this" but *enumerate it* (`LSP findReferences`, an inheritor sweep, an explicit glob), or write `unknown — not enumerated`. Two tells that a count was estimated rather than counted: a tilde or a range (`~6 edits`, `a handful`), and a round number attached to a surface nobody swept. Measured under-counts: 2 implementers that were 5; `~6 edits` that were ~40 nodes across 7 files; a "complete bounded file list" short by ~150 sites. Per `plan_file_format.md` *Counted cardinalities*, `unknown` is a legal record but never a legal bound — flag any plan claiming a bounded file list on a count it did not enumerate.
