---
description: Sync local and user permissions into the project's settings.project.json.
disable-model-invocation: true
---

Sync permissions from local/user settings into `.claude/settings.project.json`, then recompose `settings.json`.

## Purpose
When "Always allow" is clicked during a session, it writes to `settings.local.json` (gitignored) or `~/.claude/settings.json` (user-level). These don't carry over to worktrees or other workstations. This command promotes those permissions into the project overlay, `.claude/settings.project.json` — `.claude/settings.json` is a `composed` row (`baseline_sync.py compose`), generated from `settings.base.json` (tracked) plus `settings.project.json` (local), and is never edited directly (R9).

## Steps

### 1. Read all relevant settings files
- **Project composed:** `.claude/settings.json` (read-only here — the diff target; regenerated by `compose`, never edited)
- **Project overlay (the write target):** `.claude/settings.project.json`
...
### 5. Apply
- Add approved permissions to `.claude/settings.project.json` `permissions.allow` array
...
- Run `python3 .claude/tools/baseline_sync.py compose` to regenerate `.claude/settings.json` from the edited overlay
...
### 9. Summary
Report what was added (promoted, generalized, and transcript-discovered) and remind the user to commit `settings.project.json` and the recomposed `settings.json` if desired.
