...
|---|---|---|
| Named base class / interface no longer exists | Named file moved to a sibling folder; type intact | Informal naming in design pseudocode where the actual codebase entity differs in name but not shape (e.g., "Player.tscn" → real entity is `[Export] PackedScene _playerPrefab`) |
...
| "AskUserQuestion menu-pick is enough to settle a design decision" | A menu pick records user intent for a starting point. The Plan-agent deep dive must still test it against current prior art and may override the tentative shape. Treat the answer as Phase 0 input, not a substitute for design verification. |
| "The design's Open Questions are just meta-notes — load them for context, no need to surface each" | Phase 2 reads `## Open Questions`, but each unresolved scope decision must also land in the briefing's *Unresolved Scope Notes* slot. Surface every parked scope note as resolve-or-defer; otherwise the design's deferral becomes the plan's omission. |
