...
1. **Uncovered lenses are a headline, not a footnote.** Any `flags` entry of kind `lens-did-not-run` or `lens-no-return` means that dimension is UNCOVERED — not clear. Recover before re-dispatching per `orchestration` §8; the lens's spill file at `<spillDir>/<key>.spill.md` is written before validation, so a rejection leaves the deliverable intact. A dossier presented as complete with an uncovered dimension is the failure mode this whole command exists to prevent (`feedback_leak_sweep_without_adjudication_reads_as_clean`).
