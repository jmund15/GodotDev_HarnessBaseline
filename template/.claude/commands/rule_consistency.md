...
Workflow({
  ...
  args: {
    ...
    agents: [ { key: "tool-routing", prompt: "<mandate + gathered statements>", model: "<resolved native id>", effort: "<resolved effort>", agentType: "general-purpose" }, /* ≤4 per batch; resolve the fan-out pin through orchestration; never inherit the session model */ ]
  }
})
```
