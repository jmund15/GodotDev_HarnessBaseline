...
- **Not found:** create one from the schema in `common.md §6`. Default frontmatter from the calling skill's frontmatter (or user-provided for standalone). Initial Parts populated from the calling brainstorm's outputs. Proceed to Step 2.
...
If 2+ Parts have cross-roadmap deps, ALSO author the `## Cross-roadmap dependencies` section (common.md §6.12) in this same batch — it's optional schema but high-value for sub-roadmap readers.
