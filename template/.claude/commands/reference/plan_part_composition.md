...
**Failure mode this composition exists to prevent:** the no-agents limit covers only `/plan_part` verification; the drafting workflow still runs `/explore` and validation.
...
| "The design's Open Questions are just meta-notes — load them for context, no need to surface each" | Phase 2 reads the `## Open Questions` section but the extraction directive (e) + the *Unresolved Scope Notes* briefing slot exist precisely because a scope decision the design *parked* there (not resolved) is invisible otherwise. Surface every parked scope-note as resolve-or-defer; don't let the design's deferral become your omission. |
