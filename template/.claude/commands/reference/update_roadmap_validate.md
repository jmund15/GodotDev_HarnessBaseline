...
- [ ] **Sub-roadmap shape change → parent staleness** — if the target roadmap has `parent-roadmap` frontmatter (it's a sub-roadmap) AND this batch changes the Part *count* (add / split / remove) or *Pos numbering*, emit `⚠ Sub-roadmap shape changed — the parent roadmap's submap reference (bare "(N child Parts)" count) + any MVP Pos-references to this sub-roadmap may now be stale. Check the parent's submap count and MVP Pos-refs against the child; fix the parent via /update_roadmap (count) + /mvp_plan refine (MVP Pos-refs).` This **does NOT auto-edit the parent** — eager cross-roadmap mirroring is rejected by common.md §6.8 (lazy-resolution). It is a nudge only; the precise check lives in the project's cross-roadmap audit, where it keeps one, and the fix in the parent's own executors. Pure state transitions (no count/Pos change) don't fire it — they're already covered by §6.10's no-denormalize-child-state rule (the parent carries no child state to go stale).

## Disposition of warnings

If any validator fails, surface the issue in the diff preview with a `⚠` marker, fix it, and re-validate. Ask the user only when the fix is a design decision.
...
| `Sub-roadmap shape changed — parent may be stale` | After this batch lands, check the parent's submap count and MVP Pos-refs against the child (the project's cross-roadmap audit, where it keeps one). Fix the parent's bare `(N child Parts)` count via `/update_roadmap` on the parent; fix MVP Pos-references via `/mvp_plan refine MVP-N`. Do NOT denormalize the per-state breakdown back into the parent (common.md §6.10) — only the bare count is permitted to live there. |
