...
*Offline note:* `mcp__ai-worker__write_doc` is offline only when `ToolSearch("select:mcp__ai-worker__write_doc")` returns no match (the SessionStart `[ai-worker]` line prints `Ollama OK` when it is up); when offline, author the prose natively per `obsidian_conventions`, keeping this file's structure and voice rules.
