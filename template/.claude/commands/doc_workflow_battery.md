...
0. `mcp__ai-worker__*` tools are present in the session tool list. **Offline on this machine as of 2026-07-04** (ai-worker lives on another PC — see `environment_bootstrap` skill). If absent, STOP: every scenario's pass criterion depends on `write_doc` output shape; running offline produces all-FLAG results.
