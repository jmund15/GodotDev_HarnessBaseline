...
1. Pick the source:
   ...
   - planned universal work: `author start` prints a worktree on `author/<session8>`. Edit, run `python3 template/.claude/scripts/harness_tests.py` and commit there, then `publish --from-worktree <path> --dry-run`.
2. Show the owner the dry-run journal (`.claude/.cache/baseline-publish/<id>.json`): collected rows, their verdicts, and step 1–5 evidence. An invocation that already said to publish skips this pause. A dry run red in steps 1–5 is fixed and redone with `publish --resume <id> --dry-run`, then shown again; plain `--resume` refuses it.
   - Once steps 1–5 are green, run `python3 .claude/tools/baseline_linux_preview.py <id>` (in the background while the owner reviews). It runs the baseline CI's Ubuntu steps in WSL, where a Windows-green proof can still fail on POSIX paths or real symlinks. Treat a red step like a red dry run.
...
**v1 lock:** `migrate` converts it to schema 2 once; every other write refuses a v1 lock.
