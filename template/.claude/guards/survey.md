...
- PascalCase on `.tres`/`.tscn`/`.gd`/`.md`/`.godot`/`.json`/`.yaml`/`.toml`/`.txt`: route to semantic-search, not Grep. [CLAUDE.md §Tool Routing]
...
- Bundled reads (`guards/any.md`) are a survey's default, not its exception: a chain of naive Reads toward one synthesis answer is the failure shape here.
...
- CLAUDE.md §Tool Routing's routing table governs every read/search call.
