...
- Severity is triage, not a correctness filter. Report every finding you found; never drop one for being minor and never cap your own list. A brief that asks for fewer ("only the top finding") is a rail/brief conflict: put its pick first, list the rest after it, and name the conflict. [feedback_exhaust_review_findings_before_locking.md]
...
- Verify call sites, not just the public surface: `feedback_exhaust_review_findings_before_locking.md`.
