...
**Why:** wiring into a stale/orphaned target fails silently in the worst way — the new content *loads clean* (build + import green) but nothing consumes it, so the feature is dead with no error. This is distinct from the relocated-vs-missing *dependency* check ([[feedback_delegate_output_trust]]): that's about a type/file the plan *reads*; this is about the seam the plan *writes into*.

**How to apply:** trace the named target to its live consumer before editing — does anything still reference it (floor definition, autoload list, scene tree, selector array)? If orphaned, find the live equivalent; a genuine plan-fact change is halt valve (a), not a silent substitution. Cheap to check (one grep/reference-walk), expensive to miss (a shipped-but-inert feature). Related: [[gotcha_concurrent_session_hazards]], [[gotcha_plan_pending_part_already_shipped]].
