...
**Concrete:** 2026-05-13 subsystem-registry rename refactor. First proposal wired a `subsystem_drift_nudge.py` PostToolUse(Bash) hook to detect new top-level folders. User pushed back, citing context bloat and the agent's empirically observed tendency to walk past stderr nudges. Replaced with a subsystem-registry sync command invoked from `/session_end` Phase 4b, conditional on a Step 0 `git diff` gate detecting subsystem-shape or subsystem-density change. The slash command self-noops on every other session.
