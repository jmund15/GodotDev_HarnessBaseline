...
**Concrete (2026-06-04):** a pickup-strategy `.tres` shared by two player instances in different scenes (a fresh player spawns per scene) stashed `_collector`/`_inventory`/`_pullState` on itself. One player's `_ExitTree` → `Detach()` nulled the other player's collection, so **collection silently no-op'd in one scene while it worked in the other.** Symptom was maddening: "works in one scene, dead in another." Fix: strategy became stateless config; `CreateRuntime(collector, inventory)` mints a per-collector runtime. Pinned by a statelessness test (one shared strategy must hand DISTINCT runtimes to distinct collectors).
