...
**Concrete instance (2026-05-28):** a data commit resaved 20+ `.tres` files when new pipeline fields were added to Modifier scripts. It inadvertently null-stripped an int threshold Export on 7 `.tres` files. Result: `Threshold = null` → loads as 0 → `actual >= 0` is true for any value including 0 → the gate activates at 0 against an intended threshold of 1. The follow-up fix commit restored only 2 of N affected files — the remaining 5 stayed broken until detected via the threshold-regression test on a later merge gate (and the gate only caught it because the baseline-stamp-trust check exposed the regression that had been masked by a hand-stamped baseline).
...
- Prevention control (BUILT 2026-05-28): `.claude/hooks/tres_nullstrip_guard.py` flags *added* `+<Name> = null` lines on numeric-intent Export stems in staged `.tres` (diff-scoped, so already-committed nulls don't trip it) — wired into `/regression_gate` as pre-flight step `1b`. Permanent in-suite sentinel: a data-suite test textually forbids `= null` on the confirmed non-nullable-value-type-with-nonzero-default Exports. The remaining collateral (floats never restored by the `*Required`-only follow-ups) was restored to C# defaults 2026-05-28.
