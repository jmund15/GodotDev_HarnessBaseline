...
**Counterpart fact (2026-07-06) — sharing ACROSS instantiations:** the inline copy is made per **Pack**, not per **Instantiate**. Multiple `Instantiate()` calls on the SAME PackedScene object hand every instance the SAME inline sub-resource instances (Godot shares a scene's Resources across its instances unless `resource_local_to_scene`). Two consequences: (a) mutating an instance's inline Resource mutates every sibling instance's (the door.tscn multi-door last-edge-wins soft-lock — FloorSceneBuilder now deep-`Duplicate(true)`s the definition per door); (b) a test that Packs a FRESH PackedScene per consumer silently masks this class — production loads ONE cached .tscn, so production-faithful pins must reuse one PackedScene object across the consumers (FloorSceneBuilderTests `SharedDoorPackedScene` pin).

Related: [[arch_rule_test_namespace_matches_gate_filter]] (the coverage gap that hid these for a session). [[Blackboard_NullStorage_Asymmetry]] (sibling Godot-interop gotcha at the BB layer). [[arch_rule_resource_config_runtime_split]] (the structural fix for per-bind state on shared strategy Resources — TriggerBinding/CompletionBinding, 2026-07-06).
