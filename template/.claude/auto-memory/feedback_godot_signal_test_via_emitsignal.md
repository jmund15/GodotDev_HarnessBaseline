...
**Why:** a completion-rule test (`ThreeEntities_CompletesOnlyAfterAllExit`) initially set up entities under a `holder` Node3D and called `holder.RemoveChild(e); e.Free()` to simulate death. The production rule's `Bind` does `entity.TreeExiting += OnEntityExiting`. Test fired no signals because nothing was ever in a SceneTree — assertion `completed == 1` failed with `0`. The test design assumed Godot's tree mechanics worked in pure CLR; they don't.
...
**Concrete (2026-05-17, the same test):** Fixed by replacing `holder.RemoveChild(e1); e1.Free();` with `e1.EmitSignal(Node.SignalName.TreeExiting);` — test went from RED to GREEN, and the rule's `OnEntityExiting` was still validly exercised because `EmitSignal` triggers C# event subscribers.
