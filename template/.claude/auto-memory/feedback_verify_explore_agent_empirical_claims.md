...
**Scope-induced false-absence (session-diff-scoped agents).** session_audit / review-fanout agents see ONLY the session's changed files, so any "X is untested / unused / dead / orphaned" finding is a structural trap — a pre-existing test or caller in an UNCHANGED file is invisible to them (agent correct within scope, wrong about the codebase). Grep the full tree before acting on any "no test / no caller" finding. **Verified:** an agent flagged `PublishTargetMemoryToBB` "no direct test"; grep found two Logic tests in a non-session file + an integration caller.
