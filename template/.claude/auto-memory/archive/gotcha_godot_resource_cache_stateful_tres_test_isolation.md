...
**Concrete:** a trigger Resource's `_bound` one-shot latch; one `example_event.tres` loaded by two activation tests. The caching is documented Godot behavior; `CacheMode.Ignore` was applied prophylactically (not isolated via a red test).
