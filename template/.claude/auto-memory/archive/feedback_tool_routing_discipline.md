...
**Cross-references:** Global CLAUDE.md "Worker Model Delegation" section; project CLAUDE.md §9 Tool Routing — Pre-Call Litmus (the harness-wide enforcement entry); §7 (LSP); §9 (Semantic Search); the language layer's LSP-default memory.
...
- *Prompt*: "Investigate why icicles aren't shattering on hit when a fire ability hits a frozen target. Look at the icicle scene, the ability behavior, the freeze status definition, the shatter reaction resource, and any related test fixtures."
...
- *Prompt*: "Where in the codebase do we apply force impulses chain-reactively, where one entity hits another and the second entity also takes impact damage credited to the original ability caster?"
...
- *Right*: `mcp__plugin_semantic-search_semantic-search__search(query="chain force impulse collision source attribution")` first — embedding + BM25 ranking finds the collision strategy and the physics-interaction component in one shot. THEN, with names known, optional `LSP findReferences` for precise call-site enumeration.
...
- *Prompt*: "Find every place that calls AbilityCrafter.ApplyModifiers."
...
- *Right*: `LSP(operation="documentSymbol", filePath="Services/AbilityCrafter.cs", line=1, character=1)` to get the line:column of `ApplyModifiers` (line 403), THEN `LSP(operation="findReferences", filePath="Services/AbilityCrafter.cs", line=403, character=29)` for callers.
