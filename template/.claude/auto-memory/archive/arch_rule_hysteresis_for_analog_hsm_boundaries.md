...
Witnessed 2026-05-13 during Player locomotion HSM refactor — /plan_check flagged the original 0.9 single-threshold design (Walk↔Run boundary) as ASK-tier improvement. Author Walk→Run at 0.9 and Run→Walk at 0.8 in two separate magnitude-range condition `.tres` files.

**Verified (partial):** 2026-09-04 memory-claim audit — the shipped pair is asymmetric (`player_idle_to_run.tres` Min 0.9 vs `player_run_to_walk.tres` Max 0.8); the ±0.02/30-frame churn figure remains a prediction, never observed.
