...
**Evidence:** three persistence repositories (settings / meta-progression /
player-profile) were three independent top-level lines but no autoload read their
`.Instance` during boot (all consumers are UI panels, scene controllers, or
the lifecycle manager's quit path). Grouping them under a script-less `persistence.tscn` dropped the autoload count
11→9 with no ordering downside. Contrast: a transition orchestrator MUST stay a top-level line
*declared before* the lifecycle manager, because the manager's `_Ready` subscribes to
the orchestrator's `.Instance` synchronously — that ordering IS load-bearing, and reordering
breaks it silently (no crash). See [[gotcha_autoload_to_autoload_subscription_order]] and
[[gotcha_autoload_dual_registration]] (autoloads live in both `[autoload]` lines AND
