...
description: "Session-start \"no clarifying questions\" / \"make the reasonable call\" hooks govern extra-procedural pauses only; they do NOT excuse skipping a skill's load-bearing Socratic / section-by-section gates (e.g., a brainstorm skill's Socratic steps or divergence pipeline)."
metadata: 
  node_type: memory
  type: feedback
---

Session-start hooks of the shape *"work without stopping for clarifying questions"* / *"make the reasonable call and continue"* govern **extra-procedural pauses** — places where, absent any active skill procedure, you would otherwise ask "should I do X or Y" outside a defined workflow. They do **NOT** override the load-bearing gates of an invoked skill's procedure. When the user types `/<skill>` they are paying for the procedure's gates; skipping them defeats the skill's value-add and reduces the agent to a fancy wrapper around whichever tool the skill happens to invoke (`write_doc` for brainstorming, `Edit` for an authoring procedure, etc.).
...
Applies to all skills with load-bearing procedural gates — a brainstorm skill's Socratic steps and divergence pipeline (uncertainty-flag + per-cluster pacing), an authoring skill's section approval, a refactor procedure's callers-first step, a debugging skill's phase discipline, a testing skill's strict TDD RED/VERIFY/GREEN.
