...
**Concrete:** 2026-06-06 — a plan defaulted to an inline test-stub precedent in an existing suite; user flagged that the documented convention (`Tests/Framework/Mocks/` for doubles + `csharp_patterns` "eliminate duplicated test setup") made that precedent the violating exception, not the rule.
