...
**Concrete (2026-05-07):** a knocked-up state's wiring — a vertical-velocity condition observes `ICharacterController3D.IsOnFloor && Velocity.Y > MinY` **directly**, NOT `BBDataSig.<Flag> = true` + `BBFlagCondition`. The HSM observes physics state through a typed condition; no flag in the middle.
