...
**Verified:** 2026-06-14 — lock named `<owner>/wrong-baseline` (404); real repo is `<owner>/published-baseline`. The staged `harness-baseline/` dir was ahead of the remote by 22 files (18 mod + 4 new) of un-published improvements. Resolved: pushed staged→remote (remote `81d2c38`), corrected lock URL + synced_commit (`6aec8864`). A second consumer (<other-project>) had `baseline_repo` pointed at its own game repo — same class, corrected to the shared baseline. Related: [[gotcha-baseline-forked-file-drifts-behind]].
