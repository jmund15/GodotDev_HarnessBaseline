...
- *"Decide ability-vs-ability stacking semantics"* — assumed from the title to mean pierce/hit-source stacking; ran a whole Socratic decision on that. The actual Context was about **Reaction resolution** (`FindReaction` single-match vs `ResolveAll` stacking). The user answered the wrong question.
