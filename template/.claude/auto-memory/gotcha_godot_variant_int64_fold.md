...
**Concrete:** Hit 2026-05-18 in a blackboard-graph aggregate suite — three aggregate tests failed with `InvalidCastException` when written as `AggregateUp<int>(Health, 0, (acc, v) => acc + (int)v)`. Fix was test-side: switch to `long` accumulators and `(long)v` cast. Underlying Variant boxing behavior is Godot 4.x C# binding.
