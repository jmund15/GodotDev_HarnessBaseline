...
**Verified:** P6 — `EventDirector.BindDefinition` called `runScope.Bind(graph)` then `Trigger.Triggered += ActivateEvent`; integration test `DefinitionBound_EventActivatesOnRunStart` failed (event never activated) until the two lines were swapped — single-change red→green isolation. P1's seam had only been exercised by a manually-fired test trigger, so the order was never tested before.
