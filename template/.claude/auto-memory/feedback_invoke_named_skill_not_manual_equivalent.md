...
description: When the user references a named slash command (/worklog, /session_digest, /sync_baseline) and it exists in the available-skills list, invoke it via the Skill tool — don't substitute a manually-executed equivalent procedure
type: feedback
originSessionId: e9092f23-933b-4528-b9e3-ec8a6b452b62
retire_when:
  - review-by: 2026-10-20
---
When a user references a named slash command (`/worklog`, `/session_digest`, `/sync_baseline`, etc.) and that skill is in the available-skills list, invoke it via the Skill tool — even if an equivalent procedure was just executed manually in the same session.
...
**Concrete:** 2026-05-17 plan-mode session, post-plan-amendment verification step. Ran a manual 2-agent Task dispatch covering the same scope as the project's plan-audit command (memory-gotcha audit + existing-abstraction discovery), then surfaced findings as inline prose. User had to explicitly ask to run the actual command to get its canonical artifact + Phase 4 confirmation flow, despite the manual run having covered the same ground.
