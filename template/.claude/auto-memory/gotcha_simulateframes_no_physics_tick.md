...
**Corollary — MoveAndSlide position integration (verified 2026-07-12, leap-displacement repro):** synchronously pumping `MovementProcessor3D.ProcessMovement` (→ `MoveAndSlide`) in a loop preserves `Velocity` perfectly but does NOT integrate `GlobalPosition` — the transform lands only on a real physics-server step. Measured: 0.178m travel from a synchronous 31-tick pump vs 2.583m with `await body.ToSignal(tree, SceneTree.SignalName.PhysicsFrame)` interleaved between pumps, identical velocity both times. Velocity assertions are safe synchronously; displacement assertions require interleaved physics frames.
