...
**Why:** enemy death fades collected zero tweeners → corpses never freed → kill-detection (`TreeExiting`) never fired → rooms uncompletable.
...
**Verified:** log correlation (each death → Tween error) + the death-completion suite.
