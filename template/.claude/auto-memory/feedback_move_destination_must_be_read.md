...
**Why:** 2026-08-20 — a doctrine plan named `model_ladder_evidence.md` as the home for a table three times without opening it. That file already held the table, its `±` column a verbatim concatenation of the two source columns. The plan review returned it critical; the move became a merge-and-reconcile, and the reconciliation surfaced four live disagreements between the two copies.
