...
Renaming a C# file's `namespace` can silently break a bare `using X;` that only resolved because C# searches *enclosing* namespaces. A file under `{{PROJECT_NAME}}.Abilities.*` with `using Foundation;` binds `{{PROJECT_NAME}}.Abilities.Foundation`; move it to `{{PROJECT_NAME}}.Visual.*` and that lookup fails (CS0246).
...
**Fix:** before/after any namespace move (R13 structure-audit alignment), fully-qualify ancestry-dependent usings: `using {{PROJECT_NAME}}.Abilities.Foundation;`. This makes the file location-independent.
...
**Concrete:** `IAbilityOneShotEffect` ns `{{PROJECT_NAME}}.Abilities.Visual.OneShot` → `{{PROJECT_NAME}}.Visual.OneShot`, 2026-05-26 structure_audit — broke `using Foundation;`, produced 1 CS0246 + 5 cascading CS0535. Related: [[separate-preexisting-changes-before-commit]].
