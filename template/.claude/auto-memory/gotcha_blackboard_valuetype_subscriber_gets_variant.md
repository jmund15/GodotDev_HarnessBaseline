...
`Blackboard.Set<T>` stores value types (bool/int/float/vectors) as `Godot.Variant` and `NotifySubscribers` delivers **the Variant**, not a boxed CLR value. A graph/BB subscription callback pattern-matching `payload is bool b` never matches — the subscriber silently sees nothing. Reference/POCO payloads (StringName included) arrive as the raw object, which is why existing StringName-channel consumers work.
...
**Verified:** RED→GREEN 2026-07-07 — a trigger's `payload is bool` never fired on a bool latch; Variant-aware match fixed it. Sibling of [[gotcha_blackboard_stringname_variant_boxing]] / [[Blackboard_NullStorage_Asymmetry]].
