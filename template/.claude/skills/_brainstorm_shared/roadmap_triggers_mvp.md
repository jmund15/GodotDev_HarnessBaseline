...
### §6.13 Cross-roadmap index (generated)

No roadmap owns the view *across* roadmaps. A project that needs one generates it from every
`roadmap.md` — never hand-edited — and names its generator in its own roadmap command.

**That index is the live home for every cross-roadmap and child-state figure.** A parent roadmap therefore
