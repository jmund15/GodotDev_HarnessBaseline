...
**Invocation choice spends context.** Skills and commands can both be model-invocable; catalog descriptions and bodies have separate loading costs. Inspect registration, `disable-model-invocation`, path triggers and actual attachments before calling a surface deferred. Put shared detail in a reference reached at the decision that needs it.
...
**Conditionally-loaded files spend context when triggered.** A skill with 180 rules is legitimately larger than one with 40; a fixed byte cap cannot tell them apart. The unit is bytes per rule-unit (bullet, numbered step, table row, bold-lead rule — `harness_growth_guard.py` counts them mechanically and reports B/unit on every edit): **≤~300 B/unit is the ideal; >350 is the audit trigger** — the excess is narrative, restatement or evidence, and the fix is §6, not deletion of rules. Verify decision coverage before and after: every retained obligation needs an owner and test or review evidence. Duplicate, obsolete or contradictory units may be removed; preserving their count is not a quality floor. **32KB is the split trigger** regardless of density — past it, detail layers (templates, pattern catalogs, per-step mechanics) move to `skills/<name>/reference/*.md` pointed at from the exact step that reads them, and SKILL.md keeps the verdict layer. Any edit that grows a file by >1.5KB or >10% names the line the bytes outrank, or trims to match.
...
**`paths:` rules defer body loading only until their glob fires.** Prefer a prefix-anchored glob below the repo root (`Tests/**/*.cs`, `Jmodot/**`). Extension-only globs (`**/*.cs`, `**/*.md`) fire nearly every session — deferred cost, not removed. Decide width by reading the glob string, not by counting files.
