...
Keep each obligation in one canonical owner; other surfaces link to it. A triggered file never restates a rule that CLAUDE.md, CLAUDE.core.md or the runtime system prompt already carries. Inside one file, a rule that spans its steps is stated once, in the header, and no step restates it. A new incident edits that statement. It never adds a local copy at the step where the incident happened. A boundary a hook or guard enforces (a permission denial, ownership, push or commit authority) keeps its one documented home; no command or step restates it, because a restatement adds hesitation, not safety. Do not list auto-loaded siblings merely to announce their existence. Hooks may inject observed state; injected rules cite their documented owner. Tier rails by the model that RECEIVES them — rails follow the receiving model, not just the parent session.
...
For triggered bodies, ~300 bytes/rule-unit is a useful target; >350 is an audit signal, not proof of bloat. At 32KB split detail layers into `skills/<name>/reference/*.md`, cited from the step that reads them; a skill past ~800 lines or a command past ~400 also splits. Preserve required obligations and reachability, not the original rule count. Every addition receives the focused audit; growth over 1.5KB or 10% also records its reason and hard-cap response. Remove only redundant, stale, duplicated, narrative, unreachable, obvious-negation, friction or overfit text (§6), never a load-bearing rule to balance bytes. A still-exceeded hard cap splits or re-homes the detail and keeps the complete rule with its evidence. The CLAUDE.md byte band does not apply to skills; they are not always-loaded.
...
Delete whole sentences that add no needed behavior. Judge against the weakest intended receiver, and test disputed reductions on the actual task. Cut lampshading, repeated summaries, historical narration and defensive scope prose. Cut an obvious negation that restates the step's own action ("reading the file is not following it") unless a recorded failure shows receivers skip that step. Cut friction prose: an added confirmation, "only if the user explicitly asks" or "when in doubt, ask" stays only where it guards an irreversible or out-of-scope action; elsewhere the step acts. Cut an overfit aside: a clause that answers one past complaint or incident (a named file, model, run or phrasing) without a general rule behind it. Generalize it into the owning rule, or delete it. Keep a negative-scope line (`Does NOT`, `SKIP when`, a named exclusion or permission) that pre-empts a real, tempting wrong action; state it once, where the temptation arises. Keep each row of a rationalizations-to-refuse table: a row pre-empts a tempting premise even when the body already states the rule. Keep precise lookup tables and necessary decision conditions; terseness must not erase them. Keep one compressed why-clause only where it combats a counterintuitive default. A `±` table cell or one parenthetical clause is the most evidence a verdict carries inline.
...
Match the actual engine's inputs, output schema and required pins/profile. Use bounded briefs and exact evidence paths; preserve full results without forcing them into the parent. Screen against `gotcha_workflow_args_generation_fidelity`, `gotcha_workflow_fanout_search_false_absence` and machine-wide single-flight tools (one test, build or language-server run at a time); match the engine's own arg guard and output schema by reading the script. Recovery must fit the tools the receiver actually has. `orchestration` owns dispatch policy; do not duplicate model/effort tables here.
