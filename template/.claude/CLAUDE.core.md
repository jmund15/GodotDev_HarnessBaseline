...
- The CLAUDE files hold only decisions needed before a more specific trigger. Domain detail, examples and incident history belong in their triggered owner, not another standing rule.
...
Verify version-sensitive facts against `reference/project_stack.md` and first-party sources. `reference/source_trust.md` owns fetch order: cache, source-fetch script, single-page fetch, resolved library docs, multi-page synthesis, then search when needed. Inspect source coverage before asserting absence.

### 5–6. WebSearch & Git
Research search is a fallback, not a substitute for known sources. Commit verified, approved work yourself in categorical commits (feat/fix/refactor/chore, one logical concern each, path-scoped). Push feature and PR branches with an explicit refspec (`git push origin <branch>`); push main or master only when the user asked in this conversation or invoked `/commit_push` or `/clean_push`. Separate unrelated staged changes and respect submodule/peer ownership; `change_control` owns release mechanics.
...
Follow the authoring tool: snake_case for Python tooling; match neighbors for Markdown/images. File-scoped rules own each file type's conventions, harness conventions included. Comments default to none: add one only where deleting it would lead a maintainer to a wrong decision. Prefer clear contracts and authored data over special cases.

## Shell Discipline
Use `git -C <path>`; Bash cwd persists. Write Python carrying nested quotes, or backslashes inside a `claude -p` child, to a file; elsewhere a hook repairs the Bash tool's `\\`→`\` collapse on verified clients. Python file writes use LF (`newline="\n"` or bytes). Quote paths, and prefix `MSYS_NO_PATHCONV=1` for colon-bearing git/gh operands. Commit messages use a scratch file with `git commit -F`, not `/tmp`.

Watch background jobs through their completion/monitor contract. Stop an OS process only through `python3 .claude/tools/reap.py` (usage in its docstring); never kill a peer from a path substring or idle label. A rejected permission/config operation stops that operation; do not change permission mode to get around it. Provider-transport sidecars launch with `bypassPermissions`, hooks as their guards (`reference/sidecar_dispatch.md` §Permission mode). `environment_bootstrap` and `reference/sidecar_dispatch.md` own platform and launcher details.
