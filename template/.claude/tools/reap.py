...
it cannot (incident: auto-memory/archive/feedback_guard_denial_of_justified_work_fix_the_guard.md):
  ...
  reason      one closed class, each verified from evidence, not from prose:
                ...
                owner       --why quotes the owner's stop instruction VERBATIM (12+ characters; case and
                            whitespace ignored), and the quote must appear in an owner-typed message of
                            this session's transcript -- never assistant text, a subagent prompt or tool
                            output. A missing or unreadable transcript refuses. The only prose-backed class,
...
from kill_guard import _owner_inputs  # noqa: E402  (the owner-message projection a stop request reads)
...
def session_transcript(sid):
    """This session's main transcript: the single `<projects>/<key>/<sid>.jsonl`, else None."""
    if not sid or not re.fullmatch(r"[A-Za-z0-9_-]{1,128}", sid):
        return None
    hits = [p for p in PROJECTS_ROOT.glob("*/%s.jsonl" % sid) if p.is_file()]
    return hits[0] if len(hits) == 1 else None


def owner_quote(why, sid):
    """(ok, evidence): --why must appear verbatim (case/whitespace-folded) in an owner-typed message
    of THIS session's transcript. Assistant text, subagent prompts and tool output never count."""
    quote = _norm(why).strip("\"'`“”‘’ ")
    if len(quote) < OWNER_QUOTE_MIN:
        return False, ("--why must quote at least %d characters of the owner's stop instruction, "
                       "verbatim from their message" % OWNER_QUOTE_MIN)
    path = session_transcript(sid)
    inputs = _owner_inputs(str(path)) if path else None
    if inputs is None:
        return False, ("this session's transcript (session %s) is missing or unreadable, so the owner's "
                       "words cannot be verified; ask the owner to stop the process themselves, or prove "
                       "ownership under another --reason" % ((sid or "?")[:8]))
    if not any(kind == "text" and quote in _norm(text) for kind, text in inputs):
        return False, ("--why is not a verbatim quote from any owner message in this session's "
                       "transcript; copy the owner's own words (case and whitespace are ignored)")
    return True, "owner: %s" % why.strip()[:160]


def justification(args, table, pid, targets, owner=None):
    """(ok, evidence) for the claimed reason, verified mechanically. `owner` is the precomputed
    `owner_quote` verdict, read once per invocation rather than once per target."""
    reason = args.reason
    if reason == "duplicate":
        twin = args.twin
    ...
    if reason == "owner":
        return owner or (False, "owner quote was not verified")
...
def main(argv=None, env=None):
    ...
    ap.add_argument("--why", default="", help="one line for the ledger; for --reason owner, the owner's words copied verbatim from their message")
    ...
    owner = owner_quote(args.why, sid) if args.reason == "owner" and targets else None
    for pid in targets:
        row = table[pid]
        head = "%d %s :: %s" % (pid, row.get("name"), (row.get("cmd") or "")[:110])
        proof, foreign = ownership(table, pid, session_pid, sid)
        ok, evidence = justification(args, table, pid, set(targets), owner=owner)
        ...
        if not proof:
            print("REFUSED %s\n   %s" % (head, foreign or
                  ...
                  "name session %s, and no dispatch record does either%s"
                  % (session_pid, (sid or "?")[:8],
                     "\n   OWNER: %s" % evidence if args.reason == "owner" and not ok else "")))
